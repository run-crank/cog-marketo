...
describe('Cog:GetManifest', () => {
  ...
  it('should return expected step definitions', (done) => {
    cogUnderTest.getManifest(null, (err, manifest: CogManifest) => {
      ...
      // Step definitions list includes create/update lead step.
      const hasCreateOrUpdate: boolean = stepDefs.filter(s => s.getStepId() === 'CreateOrUpdateLeadByFieldStep').length === 1;
      expect(hasCreateOrUpdate).to.equal(true);

      // Step definitions list includes lead field assertion.
      const hasLeadFieldEquals: boolean = stepDefs.filter(s => s.getStepId() === 'LeadFieldEqualsStep').length === 1;
      expect(hasLeadFieldEquals).to.equal(true);

      // Step definitions list includes delete lead step.
      const hasDeleteLead: boolean = stepDefs.filter(s => s.getStepId() === 'DeleteLeadStep').length === 1;
      expect(hasDeleteLead).to.equal(true);
